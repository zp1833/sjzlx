 ...
 */
public class DeptSerialNoManager extends BaseManager {
	...
	public String getSerialNo(final Dept dept) {
		...
		log.debug("Create serial No." + serialNo);
		return serialNo;
	}
	...
	public void updateAllSerialNo() {
		...
		for (Integer id : ids) {
			Dept dept = getDao().getObject(Dept.class, id);
			getDao().evict(dept);
	...
	public void updateChildrenSerialNo(Dept parent, Map serialMap) {
		log.debug("Update " + parent.getName() + "'s children serial No.");
