...
/**
 * notes for piwik
 * site id 	: 4 -- stage.hotpotpro.com
 * 			: 5 -- cmcc.hotpotpro.com	
 */
/*var _paq = _paq || [];
...
(function() {
    var u=(("https:" == document.location.protocol) ? "https" : "http") + "://218.206.179.126/piwik/";
 ...
 * @param maxlength 验证最大长度
 * @param altType 提示框类型1.alert；2。show
 * @param alt 提示字段名（）
 */
function validateStr(id,minlength,maxlength,altType,altName){
	var str=$.trim($("#"+id).val());
	if(minlength>0){
		if(str==""){
			prt(altType,"操作失败,"+altName+"不可以为空.");
			return false;
		}
	}
	if(str.length<minlength){
		prt(altType,"操作失败,"+altName+"长度不得小于"+minlength+"位.");
		return false;
	}
	if(str.length>maxlength){
		prt(altType,"操作失败,"+altName+"长度不得大于"+maxlength+"位.");
		return false;
	}
	$("#"+id).val(str);
	return true;
 ...
 * 弹出信息方法  
 * @param alt  提示语
 * @param type 提示框类型1.alert；2。show
 */
function prt(type,alt){
	if(type==1){
		$.messager.alert("操作提示",alt,"info");
	}else if(type==2){
		$.messager.show({
				title:'操作提示',
				msg:alt,
