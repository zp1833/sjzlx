package com.alphasta.common.security.acegi.cache;
...
public class AcegiCacheManagerImpl implements AcegiCacheManager {
	...
	public void onUserChanged(User user, String oldUsername) {
		...
		log.debug("User changed " + oldUsername);
	}
	...
	private void doUserInCache(User user) {
		...
		log.debug("Modify user in cache." + user.getLoginId());
	}
	...
	public void onResourceChanged(Resource res, String oldResString) {
		if (res != null) {
			log.debug("资源" + res.getResString() + "修改了。");
		}
	...
	public void initResourceCache() {
		if (!resourceCacheInitialized) {
			List<Resource> resources = userDao.getObjects(Resource.class);
			if (resources != null) {
				for (Resource res : resources) {
					this.doResourceInCache(res);
				}
	...
	public void initUserCache() {
		if (!userCacheInitialized) {
			List<User> users = userDao.getObjects(User.class);
			for (User user : users) {
				this.doUserInCache(user);
			}
	...
	public void beforePermissionRemoved(Permission perm) {
		...
		for (User user : users) {
			...
			if (uc != null) {
				GrantedAuthority[] auths = this.removeAuthFromArray(uc
		...
		Set<Resource> resources = perm.getResources();
		log
				.debug(users.size() + " resources have Permission "
						+ perm.getName());
		if (resources != null) {
			for (Resource res : resources) {
				// 找到缓存中的Resource
				...
				if (rc != null) {
					...
					log.debug("Remove auth " + perm.getName()
							+ " from Resource " + res.getResString());
				}
			}
		}
	}
