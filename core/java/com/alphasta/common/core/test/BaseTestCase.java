...
/**
 * 测试基类. 通过继承AbstractTransactionalSpringContextTests， 
 * 使得测试执行之后，事务自动回滚。
 * <p>
 * 如果数据需要保存到数据库,在每一个测试中调用 <code>setComplete()</code>即可.
...
public class BaseTestCase extends
   ...
   */
  protected String[] locations = { "classpath*:spring/*.xml",
  ...
  public final String[] getConfigLocations() {
    // 测试的时候需要两个DataSource，一个用于测试实际项目，一个专用于测试基础类库
    // 所以这里不能用AUTOWIRE_BY_TYPE
