...
<script type="text/javascript" src="${ctx}/dwr/interface/SuccessfulCaseManager.js"></script>
<script type="text/javascript">
	...
	function removeSuccessfulCase(id) {
		if (confirm('您确定删除当前记录吗？'))
			SuccessfulCaseManager.removeSuccessfulCase(id, function() {
				ECSideUtil.reload("ec");
			});
	}
	
	function showDetail(id) {
		$("#stuDetails").attr("src","lookSuccessfulCase.do?model.id="+id);
	}
...
<body >
<div class="x-panel">
  <div class="x-panel-header">${model.toCountry }成功案例管理</div>
    <div class="x-toolbar">
      <s:form action="listSuccessfulCase" theme="simple">
	      <table width="100%">
	      	<tr>
	          	<td>
					...
					<ec:column width="40" property="_2" title="序号" value="${GLOBALROWCOUNT}" style="text-align:center" sortable="false"/>
						...
						<stc:role ifAnyGranted="ROLE_ADMIN,ROLE_ORADMIN,ROLE_DOC,ROLE_END">
							<ec:column width="60" property="_1" title="操作" style="text-align:center" viewsAllowed="html">
							   <a href="editSuccessfulCase.do?model.id=${item.id}">
							      <img src="${ctx}/images/icons/modify.gif" style="border:0px" title="编辑"/>
							   </a>
							   <a href="#" onclick="removeSuccessfulCase(${item.id})">
