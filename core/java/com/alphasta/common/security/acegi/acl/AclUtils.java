package com.alphasta.common.security.acegi.acl;
...
import com.alphasta.common.security.acegi.acl.domain.AclDomainBuilder;
import com.alphasta.common.security.acegi.acl.voter.AclDomainClass;
...
public final class AclUtils {
   ...
   * Log for the class
   */
  protected static Log log = LogFactory.getLog(AclUtils.class);
  ...
  public static AclObjectIdentity obtainIdentity(Object domainInstance,
    ...
    if (domainInstance instanceof AclObjectIdentityAware) {
      ...
      if (log.isDebugEnabled()) {
        log.debug("domainInstance: " + domainInstance
    ...
    try {
      ...
      if (log.isDebugEnabled()) {
        log.debug("Error attempting construction of "
            + defaultAclObjectIdentityClass + ": " + ex.getMessage(), ex);

        if (ex.getCause() != null) {
          log.debug("Cause: " + ex.getCause().getMessage(), ex.getCause());
        }
      }
